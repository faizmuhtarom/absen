...
function prosesAlphaOtomatis($conn) {
    ...
    // Batas waktu: Jika lewat jam 13:50 siswa belum datang, dianggap Alpha
    $batas_jam_alpha_hari_ini = "13:50:00"; 
    ...
    if ($jam_sekarang >= $batas_jam_alpha_hari_ini) {
        ...
        while($row = mysqli_fetch_assoc($cek_hari_ini)) {
            $uid = $row['rfid_uid'];
            mysqli_query($conn, "INSERT INTO log_absensi (rfid_uid, waktu, status) VALUES ('$uid', '$tgl_hari_ini $jam_sekarang', 'Alpha (Otomatis)')");
        }
    }
}
...
<body class="text-slate-800">
            ...
            <div class="flex justify-between items-center h-20">
                ...
                <div class="flex items-center gap-4">
                    <div class="relative group">
                        ...
                        <img src="gambar/logo.jpg" alt="Logo" class="relative h-12 w-12 rounded-full object-cover border-2 border-white" onerror="this.src='https://ui-avatars.com/api/?name=SMK&background=10b981&color=fff'">
                    </div>
                    <div>
                        <h1 class="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-emerald-600 to-teal-600">
                            E-Absensi RFID
                        </h1>
                        <p class="text-xs text-slate-500 font-medium tracking-wide">SMK Ma'arif 4-5 Tambakboyo</p>
                    </div>
                </div>
 ...
 <a href="scan.php" class="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium text-white bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 shadow-md shadow-emerald-200 transition-all">
                    ...
                    <?php if(isset($_SESSION['status']) && $_SESSION['status'] == "login"): ?>
                        ...
                        <a href="admin.php" class="px-4 py-2 rounded-lg text-sm font-medium text-slate-600 hover:text-emerald-600 hover:bg-emerald-50 transition-all">
                            Data Siswa
                        </a>
                        
                        <a href="rekap.php" class="px-4 py-2 rounded-lg text-sm font-medium text-slate-600 hover:text-emerald-600 hover:bg-emerald-50 transition-all">
                            Rekap
                        </a>

                        <a href="logout.php" class="ml-2 p-2 rounded-lg text-slate-400 hover:text-red-500 hover:bg-red-50 transition-all" title="Logout">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" /></svg>
                        </a>
                    <?php else: ?>
                        <a href="login.php" class="px-5 py-2 rounded-lg text-sm font-bold text-emerald-600 bg-emerald-50 hover:bg-emerald-100 transition-all">
                            Login Admin
                        </a>

                        <a href="login_siswa.php" class="px-4 py-2 rounded-lg text-sm font-medium text-slate-600 hover:text-emerald-600 hover:bg-emerald-50 transition-all">

                            Login Siswa
                        </a>
                        <a href="login_wali.php" class="px-4 py-2 rounded-lg text-sm font-medium text-slate-600 hover:text-emerald-600 hover:bg-emerald-50 transition-all">
                             Login Wali Kelas
                        </a>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 pb-12">
        ...
        <!-- Welcome Banner -->
        <div class="mb-8">
            <h2 class="text-3xl font-bold text-slate-800">Dashboard Monitoring</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
            ...
            <!-- 1. Total Siswa -->
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 card-hover transition-all duration-300 group">
                <div class="flex justify-between items-start">
                    <div>
                        <p class="text-slate-400 text-xs font-bold uppercase tracking-wider mb-1">Total Siswa</p>
                        <h3 class="text-3xl font-bold text-slate-800"><?php echo $total_siswa; ?></h3>
                    </div>
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 card-hover transition-all duration-300 group">
                <div class="flex justify-between items-start">
                    <div>
                        <p class="text-slate-400 text-xs font-bold uppercase tracking-wider mb-1">Hadir Hari Ini</p>
                        <h3 class="text-3xl font-bold text-emerald-600"><?php echo $jumlah_hadir; ?></h3>
                    </div>
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 card-hover transition-all duration-300 group">
                <div class="flex justify-between items-start">
                    <div>
                        <p class="text-slate-400 text-xs font-bold uppercase tracking-wider mb-1">Alpha / Bolos</p>
                        <h3 class="text-3xl font-bold text-purple-600"><?php echo $jumlah_alpha; ?></h3>
                    </div>
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 card-hover transition-all duration-300 group">
                <div class="flex justify-between items-start">
                    <div>
                        <p class="text-slate-400 text-xs font-bold uppercase tracking-wider mb-1">Belum Hadir</p>
                        <h3 class="text-3xl font-bold text-rose-600"><?php echo $belum_hadir; ?></h3>
                    </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50 text-slate-500 text-xs uppercase tracking-wider border-b border-slate-100">
                            ...
                            <th class="px-6 py-4 font-semibold">ID Kartu</th>
                    ...
                    <tbody class="text-sm divide-y divide-slate-100">
                        ...
                        if (mysqli_num_rows($result_log) > 0) {
                            while($row = mysqli_fetch_assoc($result_log)) {
                                ...
                                $badgeClass = "bg-slate-100 text-slate-600 border-slate-200"; // Default
                                $dotColor = "bg-slate-400";

                                // PRIORITAS 1: Ditolak / Alpha
                                if ($approval == 'Rejected' || strpos($status, 'Alpha') !== false) {
                                    $badgeClass = "bg-rose-50 text-rose-700 border-rose-100";
                                    $dotColor = "bg-rose-500";
                                }
                                // PRIORITAS 2: Pulang Awal
                                ...
                                // PRIORITAS 4: Hadir/Pulang Normal
                        ...
                        ?>
                            <tr class="hover:bg-slate-50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4">
                                    <div class="font-bold text-slate-700"><?php echo htmlspecialchars($row['nama']); ?></div>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="px-2.5 py-0.5 rounded-md text-xs font-bold bg-slate-100 text-slate-600 border border-slate-200">
